...
// The for...of loop gives each value of the array but doesn’t give you the index
...
// Suppose items = [250, 645, 300, 900, 50]:

//  In JavaScript, if you write for (let i = 0; i <= items.length; i++), the loop will try to access an index that doesn’t exist when i reaches items.length (which is 5 in this case). Here's why:

// Array Indexing: JavaScript arrays are zero-indexed. For an array of length 5, the valid indices are 0, 1, 2, 3, and 4. The element at items[5] doesn't exist and will return undefined.

// Loop Execution: When the loop reaches i = 5, the code inside the loop attempts to access items[5], which is out of bounds. If you try to perform operations like arithmetic on undefined, it may lead to NaN results or other unexpected behaviors.
